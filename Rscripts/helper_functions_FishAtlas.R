## helper_functions_FishAtlas.R
## Anne-Elise Nieblas (adapted from helper_functions.R by Sylvain Bonhommeau)
## 6/10/2017
## DESCRIPTION: This script is read by the fisheries_atlas_indicators.Rmd for the Fisheries Atlas. It loads the ss3 .nc model outputs, 
...
########################################## CatchFishStatJ DATA ######################################################
## Using FishStatJ
for_CatchFishStatJ<- function(ocean,region,species,dir_save){
  # load fishstatj catch data
  catch           <-read.csv(paste0(dir_save,'CATCH_FISHSTATJ.csv'),stringsAsFactor=FALSE)
  # nc            <-nc_open(paste(dir_save,x, sep=""))
  # exp           <-ncvar_get(nc,'Exp_Catch')
  
  # select the right catch
  this_catch<-with(catch, catch[grepl(ocean,OCEAN) & grepl(region,REGION) & grepl(species,SPECIES), ])
  
  
  
  # this_catch      <- grep(paste(catch$OCEAN==ocean & catch$REGION==region & catch$SPECIES==species & catch$FAMILY==family & catch$ORDER==order &
  #                            catch$MAJOR_GROUP==major_group)
  ...
  colnames(Catch)<-c(coldims,'Exp','Obs','Dev','stDev')
  Catch_data     <- Catch
  
  return(CPUE_data)
}

read_Catch_data <- function(model,run, dir_save){
  ...
  Catch_data    <- ddply(data_runs, .(File), function(x) data.frame(Model_run=paste(x$model, x$run),ldply(lapply(x$File, for_ncCPUE, dir_save))))[,-1]
  return(Catch_data)
}
