...
library(pacman)
p_load('shiny','plyr','reshape2','ggplot2','ncdf4','flexdashboard','plotly','devtools','Rcpp','shinyWidgets')
...
run_stats <- tibble(read.csv(paste(dirWS, "fish_atlas_stats.csv", sep=""),sep=',',stringsAsFactors = FALSE))
selectInput(inputId='Ocean',
            label='Ocean(s)',
            choices=c("Global", unique(run_stats$OCEAN)),
            selected='Global'
)
 ...
 region_subset <- reactive({ sort(unique(run_stats$REGION[which(run_stats$OCEAN==input$Ocean)])) })
  output$regionSelex <- renderUI({
    selectInput(inputId="Region", label="Region(s)", choices=region_subset(), multiple=TRUE, selected=region_subset()[1])
  })
...
#    # sort(unique(run_stats$REGION[which(run_stats$OCEAN==input$Ocean)])) 
#      if (input$Ocean=="Global"){
#     # run_stats$REGION
#        "All"
#   }else{
#     # run_stats$REGION = run_stats[grep(input$Ocean,run_stats$OCEAN),'REGION']
#     run_stats$REGION %>%      filter(str_detect(run_stats,input$Ocean)%>% select(REGION))
#   }
#    })
...
# major_group_subset <- reactive({ sort(unique(run_stats$MAJOR_GROUP[which(run_stats$OCEAN==input$Ocean & run_stats$REGION==input$Region)])) })
#   output$Major_Group <- renderUI({
#     selectInput(inputId="Major_Group", label="Major Group(s)", choices=major_group_subset(), multiple=TRUE, selected=major_group_subset()[1])
#   })
#   uiOutput("Major_Group")

output$Region <-
renderUI({
  selectInput("Region",
              label = "Region(s)",
              choices = unique(ocean.choices()$Region ),
              selected = unique(ocean.choices()$Region)[1])
})
uiOutput("Region")
# 
# region.choices = reactive({
#   if (input$Ocean=="Global"){
#     as.character(unique(run_stats$Ocean))
#   }else{
#     as.character(input$Ocean)
#   }
# })
# 
# 
# output$Major_Group <- renderUI({
#    selectizeInput("Major_Group",
#                   label="Major Group",
#                   choices =  unique(as.character(run_stats$Major_Group[which(run_stats$Ocean%in%region.choices())])),
#                   # selected=as.character(run_parameters$Run[1]),
#                   selected=unique(as.character(run_stats$Major_Group[which(run_stats$Ocean%in%region.choices())]))[1],
#                   multiple=FALSE)})
#  uiOutput("Major_Group")
# 
#  
...
#                   label="Order",
#                   choices =  unique(as.character(run_stats$Order[which(run_stats$Ocean%in%region.choices()%in%mg.choices())])),
#                   selected=unique(as.character(run_stats$Major_Group[which(run_stats$Ocean%in%region.choices()%in%mg.choices())]))[1],
#                   multiple=FALSE)})
#  uiOutput("Order")
#  
# output$Major_Group <- renderUI({
#   selectizeInput("Major_Group",
#                  label="Major Group",
#                  choices =  unique(as.character(ocean.choices()$)),
#                  # selected=as.character(run_parameters$Run[1]),
#                  selected=unique(as.character(run_stats$Major_Group[run_stats$Ocean==input$Ocean]))[1],
#                  multiple=FALSE)})
# uiOutput("Major_Group")
# # 
# output$Order <- renderUI({
#   selectizeInput("Order",
#                  label="Order",
#                  choices =  unique(as.character(run_stats$Order[run_stats$Ocean==input$Ocean])),
#                  # selected=as.character(run_parameters$Run[1]),
#                  selected=unique(as.character(run_stats$Order[run_stats$Ocean==input$Ocean]))[1],
#                  multiple=FALSE)# <------------------------------------------------------------------------------multiple may beinteresting for
# })
# uiOutput("Order")
# 
...
#                  label="Family",
#                  choices =  unique(as.character(run_stats$Family[run_stats$Ocean==input$Ocean])),
#                  # selected=as.character(run_parameters$Run[1]),
#                  selected=unique(as.character(run_stats$Family[run_stats$Ocean==input$Ocean]))[1],
#                  multiple=FALSE)# <------------------------------------------------------------------------------multiple may beinteresting for 
# })
# uiOutput("Family")
# 
# output$Stock_species <- renderUI({
#   selectizeInput("Stock_species",
#                  label="Stock species (FAO 3Alpha code)",
#                  choices =  unique(as.character(run_stats$Stock_species[run_stats$Ocean==input$Ocean])),
#                  # selected=as.character(run_parameters$Run[1]),
#                  selected=unique(as.character(run_stats$Stock_species[run_stats$Ocean==input$Ocean]))[1],
#                  multiple=FALSE)# <------------------------------------------------------------------------------multiple may beinteresting for comparison
# })
# uiOutput("Stock_species")
# 
# # output$time_step <- renderUI({
# #   selectizeInput("time_step",
# #               label="Time step (annual or monthly)",
# #               choices =  unique(as.character(run_stats$time_step[run_stats$Ocean==input$Ocean])),
# #               # selected=as.character(run_parameters$Run[1]),
# #               selected=unique(as.character(run_stats$time_step[run_stats$Ocean==input$Ocean]))[1],
# #               multiple=FALSE)# <------------------------------------------------------------------------------multiple may beinteresting for comparison
# #   })
# # uiOutput("time_step")
# # 
...
# #   selectizeInput("Year",
# #               label="Select year (gridded plots only)",
# #               choices =  unique(as.character(run_stats$Year[run_stats$Ocean==input$Ocean])),
# #               # selected=as.character(run_parameters$Run[1]),
# #               selected=unique(as.character(run_stats$Year[run_stats$Ocean==input$Ocean]))[1],
# #               multiple=FALSE)# <------------------------------------------------------------------------------multiple may beinteresting for comparison
# #   })
# # uiOutput("Year")
# # 
...
# #               label="Select month (gridded plots only)",
# #               choices =  unique(as.character(run_stats$Month[run_stats$Ocean==input$Ocean])),
# #               # selected=as.character(run_parameters$Run[1]),
# #               selected=unique(as.character(run_stats$Month[run_stats$Ocean==input$Ocean]))[1],
# #               multiple=FALSE)# <------------------------------------------------------------------------------multiple may beinteresting for comparison
# #   })
# # uiOutput("Month")
# 
...
# ```{r}
# renderPlotly({
#   Catch2plot <- Catch_plot()
#   ############# CPUE FIT: OBSERVED V PREDICTED ###########
#   p <- ggplot(na.exclude(Catch2plot), aes(x=Year, y=Obs))
