...
update_choices<-function(df){
  db1<-ddply(df,.(SPECIES, OCEAN), function(x) data.frame(qsum=sum(x$QUANTITY)))
  empty_spp<-unique(as.character(db1[which(db1$qsum==0),'SPECIES']))
  if(length(empty_spp)>0){ db<-df[-grep(paste0(empty_spp,collapse='|'),df$SPECIES),'SPECIES']  }
  return(db)
...
## effort data - using iotc data as an example
...
## data frame manipulation - effort 
...
tabPanel("Catch time series",
         ...
         sidebarLayout(
           sidebarPanel(
             ...
             pickerInput("region_select", "Ocean/Region:",   
...
server = function(input, output, session) {
  ...
  observeEvent(input$level_select, {
    ...
    if (input$level_select=="Ocean") {
      ...
      # db1<-ddply(catch_ocean,.(SPECIES, OCEAN), function(x) data.frame(qsum=sum(x$QUANTITY)))
      # empty_spp<-unique(as.character(db1[which(db1$qsum==0),'SPECIES']))
      # if(length(empty_spp)>0){ db<-catch_ocean[-grep(paste0(empty_spp,collapse='|'),catch_ocean$SPECIES),]  }
      # 
      updatePickerInput(session = session, inputId = "region_select", 
                        choices = unique(as.character(catch_ocean$OCEAN)), 
                        selected = c("Indian"))
      ...
      disabled_choices <- !unique(as.character(catch_ocean2$SPECIES)) %in% empty_spp
      # if(length(empty_spp)>0){ db<-catch_ocean[-grep(paste0(empty_spp,collapse='|'),catch_ocean$SPECIES),]  }
